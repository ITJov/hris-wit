...
// --- FUNGSI HELPER --- buat decode enum
...
const fetchAllData = async () => {
    setLoading(true);
    try {
      // Ambil data utama (projects, clients, tasks)
      ...
      // Ambil data lists based on proyek 
  ...
  const handleInsertSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/project`, payload);
      fetchAllData();
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/project/${currentProject.project_id}`, payload);
      fetchAllData();
  ...
  const handleDelete = async (project_id: string) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/project/${project_id}`);
                  ...
                  <div className="p-5 flex-grow">
                    ...
                    <div className="flex items-center space-x-2 mb-4">
                      <span className={`text-xs font-semibold px-2 py-1 rounded-full ${
                        projectStatus === 'done' ? 'bg-green-100 text-green-800' :
