...
type InsertPelamarAnakPayload struct {
...
func (payload *InsertPelamarAnakPayload) Validate() (err error) {
...
func (payload *InsertPelamarAnakPayload) ToEntity(cfg config.KVStore) (data sqlc.CreatePelamarAnakParams, err error) {
	var (
		userId = utility.GenerateGoogleUUID()
	)

	data = sqlc.CreatePelamarAnakParams{
		IDAnak:              userId,
