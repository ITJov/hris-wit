...
func (s *UsageHistoryService) DeleteUsageHistory(
	ctx context.Context,
	...
	err = q.SoftDeleteUsageHistory(ctx, sqlc.SoftDeleteUsageHistoryParams{
		UsageHistoryID: usageHistoryID,
		DeletedBy:      sql.NullString{String: deletedBy, Valid: true},
	})
	if err != nil {
		log.FromCtx(ctx).Error(err, "soft delete usage history failed")
		return errors.WithStack(httpservice.ErrUnknownSource)
	}
