...
func (s *ProjectService) InsertProject(
	ctx context.Context,
	request payload.InsertProjectPayload,
	...
	_, err = q.CreateProject(ctx, request.ToEntity(cfg, user))
	if err != nil {
		log.FromCtx(ctx).Error(err, "failed to insert project")
		return errors.WithStack(httpservice.ErrUnknownSource)
	}
