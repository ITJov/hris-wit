...
func (s *AttachmentService) InsertAttachment(
	ctx context.Context,
	request payload.InsertAttachmentPayload,
	...
	_, err = q.CreateAttachment(ctx, request.ToEntity(cfg, user))
	if err != nil {
		log.FromCtx(ctx).Error(err, "failed to insert attachment")
		return errors.WithStack(httpservice.ErrUnknownSource)
	}
