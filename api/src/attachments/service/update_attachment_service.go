...
func (s *AttachmentService) UpdateAttachment(
	ctx context.Context,
	request payload.UpdateAttachmentPayload,
	...
	_, err = q.UpdateAttachment(ctx, request.ToEntity(user))
	if err != nil {
		log.FromCtx(ctx).Error(err, "failed to update attachment")
		return errors.WithStack(httpservice.ErrUnknownSource)
	}
