...
func (s *DamageHistoryService) DeleteDamageHistory(
	ctx context.Context,
	damageHistoryID string,
	deletedBy string,
) error {
	...
	err = q.SoftDeleteDamageHistory(ctx, sqlc.SoftDeleteDamageHistoryParams{
		DamageHistoryID: damageHistoryID,
		DeletedBy:       sql.NullString{String: deletedBy, Valid: true},
	})

	if err != nil {
		log.FromCtx(ctx).Error(err, "soft delete damage history failed")
		return errors.WithStack(httpservice.ErrUnknownSource)
	}
